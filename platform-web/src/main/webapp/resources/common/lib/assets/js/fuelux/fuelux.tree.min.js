			...
			this.options.dataSource.data($el.data(), function (items) {
				...
				$.each( items.data, function(index, value) {
					...
					if(value.type === "folder") {
						...
						if("icon-class" in value){//自定义图标
							$entity.find('[class^=icon-folder-]').addClass(value["icon-class"]);
						}
		...
		selectItem: function (el) {
			...
			if (this.options.multiSelect) {
				...
				$all.removeClass('tree-selected')
					.children('i').removeClass('icon-ok').addClass('tree-dot');
				data.push($el.data());
			}
			...
			if($el.hasClass('tree-selected')) {
				...
				$el.children('i').removeClass('icon-ok').addClass('tree-dot');
			} else {
				$el.addClass ('tree-selected');
				$el.children('i').removeClass('tree-dot').addClass('icon-ok');
				if (this.options.multiSelect) {
					data.push( $el.data() );
				}
			}
